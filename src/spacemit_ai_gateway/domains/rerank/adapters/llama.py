...
class LlamaRerankAdapter:
    """llama-server --reranking 适配器（参考 LlamaEmbedAdapter）。"""
    ...
    def start(self, model_path: Path, extra_args: list[str] | None = None) -> None:
        if self._process and self._process.poll() is None:
            logger.warning("llama-server --reranking already running, stopping first")
            self.stop()
    ...
    def stop(self) -> None:
        if self._process and self._process.poll() is None:
            logger.info("Stopping llama-server --reranking (pid=%d)", self._process.pid)
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                await client.post(
                    ...
                    json={"query": "warmup", "documents": ["test"]},
                )
            logger.info("[warmup] llama-server --reranking:%d ready", self.port)
        except Exception as e:
            logger.warning("[warmup] llama-server --reranking:%d failed: %s", self.port, e)
