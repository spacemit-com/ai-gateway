...
def _get_service(request: Request) -> RerankService:
...
@router.get("/healthz")
async def rerank_healthz(request: Request):
    svc = _get_service(request)
...
async def _proxy(path: str, request: Request):
    """代理 rerank 请求到后端（llama-server 或 remote API）。"""
    svc = _get_service(request)
...
@router.post("/rerank")
async def rerank(request: Request, _: None = Depends(verify_api_key)):
    """Rerank 域的 rerank 接口（带 /v1/rerank 前缀）。"""
    return await _proxy("/v1/rerank", request)


# ── Jina AI 兼容路由 ──────────────────────────────────────────────────────────

@compat_router.get("/v1/models")
async def jina_models_compat(request: Request):
    """Jina AI 兼容的 /v1/models 接口，返回当前加载的模型。"""
    svc = _get_service(request)
...
@compat_router.post("/v1/rerank")
async def rerank_compat(request: Request, _: None = Depends(verify_api_key)):
    """Jina AI 兼容的 /v1/rerank 接口（无域前缀）。"""
    return await _proxy("/v1/rerank", request)
